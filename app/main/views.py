#coding:utf-8
from flask import render_template, request, current_app, redirect, url_for, flash, send_from_directory
import os
from . import main
from ..models import Article, ArticleType, article_types, Comment, Follow, User, Source, BlogView
...
def index():
    ...
    pagination = Article.query.order_by(Article.create_time.desc()).paginate(
            page=page, per_page=current_app.config['ARTICLES_PER_PAGE'],
            error_out=False)
...
def serve_img(filename):
    img_path = os.path.join(current_app.root_path, '..', 'img')
